...
def preprocess_image(image):
    ...
    Redimensiona para 224x224 se 'mobilenet' estiver no nome do modelo.
    Caso contrário, assume (200x66) e normalização padrão NVIDIA.
    """
    global args
    ...
    if "mobilenet" in args.model.lower():
        ...
        image = image.astype(np.float32) / 127.5 - 1.0  # Normalização MobileNet
    else:
        image = cv2.cvtColor(image, cv2.COLOR_RGB2YUV)
        image = cv2.resize(image, (200, 66))
        image = image / 255.0  # Normalização NVIDIA
