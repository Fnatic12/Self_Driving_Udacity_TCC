...
def plot_training_log(csv_path='training_log.csv', save_dir='plots'):
    ...
    # Gráfico 2: Steering
    if {'loss_steering', 'val_loss_steering'}.issubset(available_columns):
        plt.figure(figsize=(8, 5))
        plt.plot(df['loss_steering'], label='Train Steering')
        plt.plot(df['val_loss_steering'], label='Val Steering')
        plt.xlabel('Época')
        plt.ylabel('Loss (Steering)')
        plt.title('Evolução da Loss - Steering')
        plt.legend()
        plt.grid(True)
