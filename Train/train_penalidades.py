...
def custom_steering_loss(alpha=0.1, threshold=0.5):
    def loss_fn(y_true, y_pred):
        mse = MeanSquaredError()(y_true, y_pred)
        penalty = tf.where(tf.abs(y_pred) > threshold, tf.square(y_pred), tf.zeros_like(y_pred))
        return mse + alpha * tf.reduce_mean(penalty)
    return loss_fn

def custom_throttle_loss(beta=0.05, accel_threshold=0.2):
    def loss_fn(y_true, y_pred):
        mse = MeanSquaredError()(y_true, y_pred)
        accel = tf.abs(y_pred[:, 1:] - y_pred[:, :-1])
        accel_penalty = tf.where(accel > accel_threshold, tf.square(accel), tf.zeros_like(accel))
        total_penalty = tf.reduce_mean(accel_penalty)
        return mse + beta * total_penalty
    return loss_fn
...
def build_model_nvidia():
    ...
    model.compile(
        ...
        loss={
            'steering': custom_steering_loss(alpha=0.1, threshold=0.5),
            'throttle': custom_throttle_loss(beta=0.05, accel_threshold=0.2)
        },
...
def train_model(model, args, X_train, X_valid, y_train, y_valid):
    log_name = f"training_log_penalidades{args.model_name}.csv"
    csv_logger = CSVLogger(log_name, append=True)
    checkpoint = ModelCheckpoint(f"model_penalidades_{args.model_name}-{{epoch:03d}}.h5", monitor='val_loss', save_best_only=args.save_best_only, mode='auto', verbose=1)
    ...
    model_name = f"model_penalidades{args.model_name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.h5"
